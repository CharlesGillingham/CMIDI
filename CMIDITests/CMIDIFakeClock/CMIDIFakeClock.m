...
@interface CMIDIFakeClock ()
@end


@implementation CMIDIFakeClock {
...
+ (CMIDIClock *) fakeClock {   return (CMIDIClock *)[[self alloc] init];  }

- (void) runFakeClock
{
    CMIDINanoseconds now = _hostTimeOfCurrentTick;
    while (YES) {
        if (!_isRunning) {
            return;
        }
        [self timerDone:now];
        now += _nanosecondsPerTick;
    }
...
- (void) notifyStart
{
    ...
    for (NSObject <CMIDITimeReceiver> * receiver in receivers) {
        if ([receiver respondsToSelector:@selector(clockStarted:)]) {
            [receiver clockStarted:(CMIDIClock *)self];
        }
    }
...
- (void) notifyStop
{
    for (NSObject <CMIDITimeReceiver> * receiver in receivers) {
        if ([receiver respondsToSelector:@selector(clockStopped:)]) {
            [receiver clockStopped:(CMIDIClock *)self];
        }
    }
    ...
    if (_tempoNeedsNotify) {
        ...
        for (NSObject <CMIDITimeReceiver> * receiver in receivers) {
            if ([receiver respondsToSelector:@selector(clockTempoSet:)]) {
                [receiver clockTempoSet:(CMIDIClock *)self];
            }
        }
    ...
    if (_tickNeedsNotify) {
        ...
        for (NSObject <CMIDITimeReceiver> * receiver in receivers) {
            if ([receiver respondsToSelector:@selector(clockTimeSet:)]) {
                [receiver clockTimeSet:(CMIDIClock *)self];
            }
        }
...
//        [_timer sendMessageAtHostTime:AudioConvertNanosToHostTime(_hostTimeOfCurrentTick + _nanosecondsPerTick)];
 //   }
    ...
    for (NSObject <CMIDITimeReceiver> * receiver in receivers) {
        [receiver clockTicked:(CMIDIClock *)self];
    }
...
@interface CMIDIFakeClock (KVC)
@end

@implementation CMIDIFakeClock (KVC)
